...
import { Send, CheckCheck, Check, Paperclip, Smile, Search, MoreVertical, FileText, X, Download } from 'lucide-react'
...
export default function ChatWindow({ receiverId, receiverName, orderId, orderContext }: ChatWindowProps) {
  ...
  // Upload progress tracked for future upload UI
  const [_uploadProgress, setUploadProgress] = useState(0) // NOSONAR - value will be used when upload UI is implemented
  const [showSearch, setShowSearch] = useState(false)
  const [searchQuery, setSearchQuery] = useState('')
  // Typing indicator tracked for future UI
  const [_isTyping, setIsTyping] = useState(false) // NOSONAR - value will be used when typing indicator UI is implemented
  ...
  const handleFetchSuccess = useCallback((newMessages: Message[], currentUserId: string) => {
    ...
    if (previousMessageCountRef.current > 0) {
      const hasNewFromReceiver = newMessages.some((m: Message) => 
        m.senderId === receiverId && !messages.some(oldMsg => oldMsg.id === m.id)
      )
