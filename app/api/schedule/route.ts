...
/**
 * GET /api/schedule
 * Obtener horarios de pedidos del vendedor autenticado
 */
  ...
  try {
    ...
    if (!seller) {
      return NextResponse.json({ 
        error: 'Solo vendedores pueden acceder a horarios' 
      }, { status: 403 })
    }
    ...
    logger.info(LogCategory.API, 'Order schedules retrieved', { 
      sellerId: seller.id,
    ...
    logger.error(LogCategory.API, 'Error retrieving order schedules', error)
    return NextResponse.json({
      success: false,
...
/**
 * PUT /api/schedule
 * Configurar horarios de pedidos para el vendedor
 * ✅ IDEMPOTENTE (upsert)
 * 
 * Body example:
 * {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!seller) {
      return NextResponse.json({ 
        error: 'Solo vendedores pueden configurar horarios' 
      }, { status: 403 })
    }
    ...
    // ✅ Validación con Zod
    const validation = validateSchema(scheduleSchema, body)
    ...
    logger.info(LogCategory.API, 'Order schedules updated', {
      sellerId: seller.id,
    ...
    const updatedSchedules = await prisma.orderSchedule.findMany({
      where: { sellerId: seller.id },
      orderBy: { dayOfWeek: 'asc' }
    })
    ...
    logger.error(LogCategory.API, 'Error updating order schedules', error)
    return NextResponse.json({
      success: false,
