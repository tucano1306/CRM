...
export default function SellerSignInPage() {
  ...
  const checkRoleConflict = async () => {
    setChecking(true);
    try {
      const response = await fetch('/api/auth/check-role-conflict?role=SELLER');
      ...
      if (data.hasConflict) {
        ...
        // No hay conflicto, continuar a productos
  ...
  // Mostrar mensaje de conflicto si el usuario ya es comprador
            ...
            <div className="space-y-3">
              <p className="text-sm text-gray-600 text-center">
                Tu rol actual es: <strong className="text-orange-600">
                  {roleConflict.currentRole === 'CLIENT' ? '🛒 Comprador' : '🏪 Vendedor'}
                </strong>
              </p>
              
              <button
                onClick={() => router.push('/buyer/sign-in')}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 px-4 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
              >
                <ArrowLeft className="w-4 h-4" />
                Ir al Login de Comprador
              </button>
